...
if (FALSE){
  ...
  # for app
  file.copy("Data/05_dat_status_trend_2025.rds", 
...
### SAVE ### 
# saveRDS(meta_status_region, "Data/06_meta_status_region_2025.rds")
...
if (TRUE){
  ...
  ggsave("Figures/Relative_class_CB118_by_MSFDregion2_2025.png", gg2, width = 9.5, height = 5, dpi = 400)
  ggsave("Figures/2025/Relative_class_CB118_by_MSFDregion2_2025.svg", gg2, width = 9.5, height = 5, dpi = 400)
...
gg_boxplot <- plot_relclass_param2("CB118", msfd_regions = TRUE,
                                   binwidth = 0.15, overflow = "compress", 
                                   get_boxplot_only = TRUE)
...
plotdata_boxplot %>%
  as.data.frame() %>%
  writexl::write_xlsx("Figures/2025/PCB118_by_MSFDregion_boxplot_fromR_2025.xlsx")

ggplot(plotdata_boxplot, aes(x_label)) +
  geom_point(aes(y=`lower end of bar`), shape = 6) +
  geom_point(aes(y=`lower end of box`), shape = 6, color = "red2") +
  geom_point(aes(y=`line inside box`), shape = 15, color = "blue3") +
  geom_point(aes(y=`upper end of box`), shape = 2, color = "red2") +
  geom_point(aes(y=`upper end of bar`), shape = 2)
...
# point data
# debugonce(plot_relclass_param2)
gg_points <- plot_relclass_param2("CB118", msfd_regions = TRUE,
                                  binwidth = 0.15, overflow = "compress", 
                                  get_points_for_data = TRUE)

dd_points <- layer_data(gg_points)
...
dd_points <- ggplot_build(gg_points)$data[[1]] %>% 
  mutate(
    Measurement = factor(
      ifelse(colour == "grey50", "Under LOQ", "Quantified"),
...
stackdist_for_delivery <- 0.04
list_of_pointdata <- list(
  dd_points = dd_points %>% 
    as.data.frame(),
  points = dd_points %>% 
    mutate(
      x = x + stackdist_for_delivery*stackpos,
      Shape = case_when(
        Colour %in% "grey" ~ "triangle pointing down",
        TRUE ~ "round"))
)

writexl::write_xlsx(list_of_pointdata, "Figures/2025/PCB118_by_MSFDregion_dots_fromR_2025.xlsx")

stackdist_fig <- 0.05
cols = c("1" = "green3", "3" = "red", "2" = "yellow2", "NA" = "grey50")
ggplot(dd_points, aes(x + stackdist_fig*stackpos, y, fill = Status))+
  geom_point(colour = "black", shape = 21) +     # see ?points for shape values
...
if (TRUE){
  ...
  ggsave("Figures/2025/Relative_class_HG_by_MSFDregion.png", gg2, width = 8, height = 5, dpi = 400)
...
# based on code in script 11  
lookup_param_mapname <- data.frame(
  PARAM = c("CD","CU","PB","HG","HCB","HCHG","DDEPP","BAP", "CB118"),
  PARAM_commonname = c("Cd","Cu","Pb","Hg","HCB","HCHG","p,p'-DDE","BAP", "CB118")
)

tab1 <- dat_status_trend %>%
  left_join(lookup_param_mapname, by = "PARAM") %>% 
  mutate(
    PARAM_commonname = factor(PARAM_commonname,
                              levels = lookup_param_mapname$PARAM_commonname)
  ) %>%
  tabyl(Trend, PARAM_commonname)
...
# Combine
tab_report_1 <- bind_rows(tab2a,  tab2b[5,])

# Fix first column
lookup_column1 <- data.frame(
  Trend_old = c("Decrease", "Increase", "No trend", 
            "Trend unknown", "Total"),
  Trend = c("Decrease (% of stations)", "Increase (% of stations)", "No trend (% of stations)", 
              "Trend unknown (% of stations)", "Number of stations assessed")
) 

tab_report <- tab_report_1 %>%
  rename(Trend_old = Trend) %>% 
  left_join(lookup_column1, by = join_by(Trend_old)) %>% 
  select(Trend_old, Trend, everything()) 

# Write to excel for export to assessment 
writexl::write_xlsx(tab_report %>% select(-Trend_old), "Figures/2025/EXTRAFIG-MAR001.xlsx")
...
saveRDS(meta_trends_region, "Data/06_meta_trends_region_2025.rds")
saveRDS(meta_trends_msfdregion, "Data/06_meta_trends_msfdregion_2025.rds")
...
if (TRUE){
  ...
  ggsave("Figures/2025/Meta_trend_HG_by_MSFDregion.png", gg, width = 6, height = 3, dpi = 400)
...
if (TRUE){
  ...
  ggsave("Figures/2025/Meta_trend_DDEPP_by_MSFDregion.png", gg, width = 6, height = 3, dpi = 400)
...
if (TRUE){
  ...
  ggsave("Figures/2025/Meta_trend_CB118_by_MSFDregion.png", gg, width = 6, height = 3, dpi = 400)
