...
df_thresholds <- readxl::read_excel("Input_data/MGR_05_df_thresholds.xlsx", sheet = "MGR")
...
if (TRUE){
  ...
  ggsave("Figures/Relative_class_CB118_by_MSFDregion.png", gg2, width = 9.5, height = 5, dpi = 400)
...
if (TRUE){
  ...
  #
  # boxplot data
  gg2 <- plot_relclass_param("CB118", msfd_regions = TRUE, get_boxplot_only = TRUE)
  dd <- layer_data(gg2)
  ...
  plotdata_boxplot <- dd %>%
    select(x, ymin, lower, middle, upper, ymax, new_width) %>%
    mutate(MSFD_region = x_labels[x], .before = 1)
  ...
  plotdata_points <- dat_status_trend_relclass %>%
    ...
    rename(x_label = MSFD_region, y = Relclass, colour = Status) %>%
    mutate(x = as.numeric(x_label)) %>%
    as.data.frame()
  ...
  #
  # Plot data saved to this excel file:
  # "Figures/06_Relative_class_CB118_by_MSFDregion_plotdata.xlsx" 
  # - this is done a bit further down in this script, in 
  #   'Trend meta-analysis by MSFD region' - CB118
  #
