...
dat_1_all <- readRDS("Data/03_dat_7_2025.rds")
...
* Decide to delete basaed on the number and the spread of locations from map 

```{r}

stations_with_several_msfdregions <- dat_1_clean1 %>%
  distinct(id, Country, MSFD_region) %>% 
  add_count(id, Country) %>% 
  filter(n > 1)

stations_with_several_msfdregions

lookup_position <- dat_1_clean1 %>%
  distinct(id, Country, MSFD_region, Longitude, Latitude)
...
if (FALSE){  
  # the spread of locations from map   
...
#### Delete stations with large spread and several MSFD regions  

* One Danish station  

```{r}

stations_to_remove <- dat_1_clean1 %>%
  distinct(id, Country, MSFD_region) %>% 
  # number of MSFD regions per station:
  count(id, Country) %>% 
  # keep stations with > MSFD region per station - these wiull be deleted
  filter(n > 1)

dat_1 <- dat_1_clean1 %>% 
  left_join(stations_with_several_msfdregions) %>% 
  filter(is.na(n)) %>% 
  select(-n)
...
dat_medians_from_duplicates <- readRDS("Data/03_df_data_from_dupl_2025.rds")

# df_duplicates <- readRDS(file = "Data/03_df_check_all_2024.rds")
...
### Check that there is one MSFD_region per station 
...
filename_medians <- "Data/04_dat_medians_2025.rds"
