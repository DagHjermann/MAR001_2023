...
dat_a_old <- readRDS("Data/01_Combined_data_with_duplicates_a.rds")
dat_a <- readRDS("Data/01_Combined_data_with_duplicates_a_with_PT.rds")

cat("nrow(dat_a): ", nrow(dat_a), "\n")
cat("nrow difference: ", nrow(dat_a) - nrow(dat_a_old), "\n")

# dat_b <- readRDS("Data/01_Combined_data_with_duplicates_b.rds")
...
saveRDS(df_bivalve_dryweight, "Data/03_df_bivalve_dryweight.rds")
...
saveRDS(dat_6, "Data/03_dat_6.rds")
...
df_check_all_filename <- "Data/03_df_check_all_1a.rds"
...
df_check_all_1a <- df_check_all_1a %>%
  ...
  mutate(n_year = length(unique(year)),
         n_year_2010.20 = length(unique(year[year >= 2010 & year <= 2020])),
         n_year_2005.20 = length(unique(year[year >= 2005 & year <= 2020])),
         last_year = max(year),
...
if (TRUE){
  saveRDS(df_data_from_dupl, "Data/03_df_data_from_dupl.rds")
}

```


## 8. APPENDIX Duplicate check, name missing  
...
## 9. More checking of df_check_all2 (no changes)  
