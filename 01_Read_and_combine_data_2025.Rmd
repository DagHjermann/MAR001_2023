...
# For checking data, read just 3 rows:
 fn <- "Input_data/From_Hans_2025/EMD_biota_2025.txt"
...
fn <- "Input_data/From_Hans_2025/EMD_biota_2025_2DECIMALS.txt"
...
### Check MSFD_region
* Should be only one MSFD_region per station  

#### EMODnet  

```{r}

stations_with_several_msfdregions <- df1a %>%
  distinct(StationName, SD_StationCode, SD_StationName, Country, MSFD_region) %>% 
  add_count(StationName, SD_StationCode, Country) %>% 
  filter(n > 1)

stations_with_several_msfdregions

lookup_position <- df1a %>%
  distinct(StationName, SD_StationCode, Country, MSFD_region, Longitude, Latitude)

stations_with_several_msfdregions_loc <- stations_with_several_msfdregions %>% 
  left_join(lookup_position, na_matches = "na", relationship = "many-to-many")
cat(nrow(stations_with_several_msfdregions_loc), "positions\n")

if (FALSE){  
  # copy-paste to console for map
  library(leaflet)
  leaflet(stations_with_several_msfdregions_loc) %>% 
    addTiles() %>% 
...
stations_with_several_msfdregions <- df2_orig %>%
  distinct(STATN, SD_StationCode, SD_StationName, Country, MSFD_region) %>% 
  add_count(STATN, SD_StationCode, Country) %>% 
  filter(n > 1)

stations_with_several_msfdregions

lookup_position <- df2_orig %>%
  distinct(STATN, SD_StationCode, Country, MSFD_region, Longitude, Latitude)
...
saveRDS(dat_a, "Data/01_Combined_data_with_duplicates_a_2025.rds")
saveRDS(dat_b, "Data/01_Combined_data_with_duplicates_b_2025.rds")
